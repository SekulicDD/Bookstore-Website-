...
if(isset($_POST['id'])&&isset($_POST['formData'])){
    ...
    if(!preg_match("/^\w+.jpg$/",$image)){
        array_push($errors,"Image needs to be in jpg format");
    }
    ...
    if(count($errors)>0){
        ...
        $result = $conn->prepare("UPDATE books SET name=?,Author=?,text=?,price=?,inStock=?,image=?,isFeatured=? WHERE id=?");
        $result->bindValue(1, $name);
        $result->bindValue(2, $author);
        $result->bindValue(3, $text);
        $result->bindValue(4, $price);
        $result->bindValue(5, $stock);
        $result->bindValue(6, $image);
        $result->bindValue(7, $featured);
